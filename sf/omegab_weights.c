...
*omegabDataIn, *omegabDataGet;
           
/* define properties to be injected */
void particle2in_omegab(struct omegab_data_in *in, int i);
void particle2in_omegab(struct omegab_data_in *in, int i)
{
...
struct omegab_data_out
{
	MyDouble omegab_p[3];
	MyDouble omegab_m[3];
}
*omegabDataResult, *omegabDataOut;
...
int omegab_evaluate_active_check(int i)
{
...
{
#define CONDITION_FOR_EVALUATION if(omegab_evaluate_active_check(i)==1)
#define EVALUATION_CALL omegab_evaluate(i, 0, exportflag, exportnodecount, exportindex, ngblist)
...
{
#define EVALUATION_CALL omegab_evaluate(j, 1, &dummy, &dummy, &dummy, ngblist);
...
void omegab_calc(void)
{
    ...
    All.BunchSize = (int) ((MyBufferSize * 1024 * 1024) / (sizeof(struct data_index) + sizeof(struct data_nodelist) +
                                             sizeof(struct omegab_data_in) + sizeof(struct omegab_data_out) + sizemax(sizeof(struct omegab_data_in),sizeof(struct omegab_data_out))));
    ...
    do
    {
        ...
        {
            threadid[j] = j + 1;
            pthread_create(&mythreads[j], &attr, omegab_evaluate_primary, &threadid[j]);
        }
#endif
#ifdef _OPENMP
#pragma omp parallel
#endif
        {
            ...
            int mainthreadid = 0;
#endif
            omegab_evaluate_primary(&mainthreadid);	/* do local particles and prepare export list */
        }
        ...
        omegabDataGet = (struct omegab_data_in *) mymalloc("omegabDataGet", Nimport * sizeof(struct omegab_data_in));
        omegabDataIn = (struct omegab_data_in *) mymalloc("omegabDataIn", Nexport * sizeof(struct omegab_data_in));
        ...
        for(j = 0; j < Nexport; j++)
        {
            ...
            particle2in_omegab(&omegabDataIn[j], place);
            memcpy(omegabDataIn[j].NodeList,
        ...
        for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
            ...
            if(recvTask < NTask)
            {
                ...
                {
                    /* get the particles */
                    MPI_Sendrecv(&omegabDataIn[Send_offset[recvTask]],
                                 Send_count[recvTask] * sizeof(struct omegab_data_in), MPI_BYTE,
                                 recvTask, TAG_TO_USE,
                                 &omegabDataGet[Recv_offset[recvTask]],
                                 Recv_count[recvTask] * sizeof(struct omegab_data_in), MPI_BYTE,
        ...
        myfree(omegabDataIn);
        omegabDataResult = (struct omegab_data_out *) mymalloc("omegabDataResult", Nimport * sizeof(struct omegab_data_out));
        omegabDataOut = (struct omegab_data_out *) mymalloc("omegabDataOut", Nexport * sizeof(struct omegab_data_out));
        
        /* now do the particles that were sent to us */
        NextJ = 0;
#ifdef PTHREADS_NUM_THREADS
        for(j = 0; j < PTHREADS_NUM_THREADS - 1; j++)
            pthread_create(&mythreads[j], &attr, omegab_evaluate_secondary, &threadid[j]);
#endif
#ifdef _OPENMP
#pragma omp parallel
#endif
        {
            ...
            int mainthreadid = 0;
#endif
            omegab_evaluate_secondary(&mainthreadid);
        }
        ...
        for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
            ...
            if(recvTask < NTask)
            {
                ...
                {
                    /* send the results */
                    MPI_Sendrecv(&omegabDataResult[Recv_offset[recvTask]],
                                 Recv_count[recvTask] * sizeof(struct omegab_data_out),
                                 MPI_BYTE, recvTask, TAG_TO_USE,
                                 &omegabDataOut[Send_offset[recvTask]],
                                 Send_count[recvTask] * sizeof(struct omegab_data_out),
        ...
        {
            place = DataIndexTable[j].Index;
            out2particle_omegab(&omegabDataOut[j], place, 1);
        }
        myfree(omegabDataOut);
        myfree(omegabDataResult);
        myfree(omegabDataGet);
    }
    while(ndone < NTask);
...
int omegab_evaluate(int target, int mode, int *exportflag, int *exportnodecount, int *exportindex, int *ngblist)
{
    ...
    struct omegab_data_in local;
    struct omegab_data_out out;
    memset(&out, 0, sizeof(struct omegab_data_out));
    //kernel_main(0.0,1.0,1.0,&kernel_zero,&wk,-1);
	//out.weight_tot = 0;
    
	/* Load the data for the particle injecting feedback */
    if(mode == 0) {particle2in_omegab(&local, target);} else {local = omegabDataGet[target];}
    ...
    {
        startnode = omegabDataGet[target].NodeList[0];
    ...
    while(startnode >= 0)
    {
        ...
        { 
            ...
            for(n = 0; n < numngb_inbox; n++)
            {
				...
				//construct the effective faces of the interacting gas elements
				double Xba[3], Xba_p[3], Xba_m[3], Xba_vers[3]; //gas particle position as seen from the star
        ...
        if(mode == 1)
        {
            ...
            {
                startnode = omegabDataGet[target].NodeList[listindex];
                if(startnode >= 0) {startnode = Nodes[startnode].u.d.nextnode;}	/* open it */
            }
        } // if(mode == 1)
    } // while(startnode >= 0)
    /* Now collect the result at the right place */
    if(mode == 0) {out2particle_omegab(&out, target, 0);} else {omegabDataResult[target] = out;}
    return 0;
} // int omegab_evaluate
