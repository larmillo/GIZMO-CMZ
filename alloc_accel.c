...
{
#ifndef GRADUAL_NO_AXISYMMETRIC_POTENTIAL	
...
#else
    hid_t   file, dataset_pot, dataspace_pot, dataset_potbar, dataspace_potbar, dataset_potsymbar, dataspace_potsymbar, datatype;
	hid_t   dataset_x, dataset_y, dataset_z, dataspace_x, dataspace_y, dataspace_z;
    size_t      size;                    /* size of the data element stored in file */
    hsize_t     dims_out[3];             /* dataset dimensions */
    herr_t      status=0;
    ...
    //file = H5Fopen("MWpotential.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
	file = H5Fopen("NewPotential_FineGrid.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
	
    dataset_pot = H5Dopen(file, "/Potentials/Symmetric");
	dataset_potbar = H5Dopen(file,"/Potentials/Bar");
	dataset_potsymbar = H5Dopen(file,"/Potentials/BarSym");
    dataspace_pot = H5Dget_space(dataset_pot);
	dataspace_potbar = H5Dget_space(dataset_potbar);
	dataspace_potsymbar = H5Dget_space(dataset_potsymbar);
    ...
    All.potential = malloc(Nx*Ny*Nz*sizeof(double));
	All.potential_bar = malloc(Nx*Ny*Nz*sizeof(double));
	...
	{
		All.potential[i]*=1e10/(All.UnitVelocity_in_cm_per_s * All.UnitVelocity_in_cm_per_s);
		All.potential_bar[i]*=1e10/(All.UnitVelocity_in_cm_per_s * All.UnitVelocity_in_cm_per_s);
		All.potential_symbar[i]*=1e10/(All.UnitVelocity_in_cm_per_s * All.UnitVelocity_in_cm_per_s);
	}	
...
void allocate_acc_coarsegrid()
{
    ...
    //file = H5Fopen("MWpotential.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
	file = H5Fopen("NewPotential_CoarseGrid.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
    ...
    All.coarse_potential = malloc(Nx*Ny*Nz*sizeof(double));
	All.coarse_potential_bar = malloc(Nx*Ny*Nz*sizeof(double));
	...
	for(int i=0; i < Nx*Ny*Nz; i++) 
	{
		All.coarse_potential[i]*=1e10/(All.UnitVelocity_in_cm_per_s * All.UnitVelocity_in_cm_per_s);
		All.coarse_potential_bar[i]*=1e10/(All.UnitVelocity_in_cm_per_s * All.UnitVelocity_in_cm_per_s);
		All.coarse_potential_symbar[i]*=1e10/(All.UnitVelocity_in_cm_per_s * All.UnitVelocity_in_cm_per_s);
	}	
